...
export class SessionStore {
  ...
  getOrCreateIsolate(): Isolate {
    if (!this.isolate) {
      // Lazy require — only load the native module when a Code block actually
      // executes. This prevents isolated-vm from crashing the entire ORPC
      // route handler at module-evaluation time when it is not available.
      // eslint-disable-next-line @typescript-eslint/no-require-imports
      const { Isolate } = require("isolated-vm") as typeof import("isolated-vm");
