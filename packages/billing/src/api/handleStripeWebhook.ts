...
export const handleStripeWebhook = async ({
  ...
  switch (event.type) {
    ...
    case "invoice.paid": {
      ...
      if (providerInvoiceId) {
        await prisma.orgInvoice.upsert({
          ...
          update: { status: "PAID", pdfUrl },
        });

        const periodEnd = (invoice as Stripe.Invoice & { period_end?: number })
          .period_end;
        ...
        await prisma.workspace.update({
          ...
          data: { status: "ACTIVE", isPastDue: false },
        });
      }

      return { message: "invoice.paid recorded" };
    }
    ...
    case "invoice.payment_failed": {
      ...
      return { message: "invoice.payment_failed recorded" };
    }
