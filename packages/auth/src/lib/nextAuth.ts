...
import { getCachedDbProviders } from "./dbProviders";
import { providers } from "./providers";

export const SET_TYPEBOT_COOKIE_HEADER = "Set-Typebot-Cookie" as const;

const nextAuth = NextAuth((req) => {
  // Merge env-var providers (always active) with DB-configured providers
  // (toggled on/off by superadmin). Env-var providers take precedence —
  // if the same provider ID appears in both, only the env-var one is used.
  const envProviderIds = new Set(providers.map((p) => (p as { id: string }).id));
  const dbProviders = getCachedDbProviders().filter(
    (p) => !envProviderIds.has((p as { id: string }).id),
  );
  return {
  adapter: createAuthPrismaAdapter(prisma),
  secret: env.ENCRYPTION_SECRET,
  providers: [...providers, ...dbProviders],
