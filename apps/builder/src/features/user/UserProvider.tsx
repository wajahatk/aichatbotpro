...
export const UserProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    ...
    const isPathPublicFriendly = /\/chatbots\/.+\/(edit|theme|settings)/.test(
      router.pathname,
    );
    const isMarketingPath =
      router.pathname === "/" ||
      ["/pricing", "/features", "/about"].includes(router.pathname) ||
      router.pathname.startsWith("/legal/");
    if (isSignInPath || isPathPublicFriendly || isMarketingPath) return;
    if (status === "unauthenticated")
      router.replace({
        pathname: "/signin",
        query:
          router.asPath !== "/chatbots"
            ? {
