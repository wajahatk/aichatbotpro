...
export const handleCreateTypebot = async ({
  ...
  if (workspace.subscriptionPlan) {
    const currentBotCount = await prisma.typebot.count({
      where: { workspaceId, isArchived: false },
    });
    if (currentBotCount >= workspace.subscriptionPlan.maxBots)
      throw new ORPCError("FORBIDDEN", {
        message: `Your ${workspace.subscriptionPlan.name} plan allows up to ${workspace.subscriptionPlan.maxBots} bots. Please upgrade to create more.`,
      });
  }
