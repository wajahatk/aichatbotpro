...
describe("googleSheetsOAuthState", () => {
  it("round trips a signed OAuth state", () => {
    const input = googleSheetsOAuthContextSchema.parse({
      redirectUrl: "https://app.typebot.io/chatbots/typebot-id/edit?foo=bar",
    ...
    expect(parsedState).toMatchObject({
      userId: "user-id",
      workspaceId: "workspace-id",
      redirectPath: "/chatbots/typebot-id/edit",
      typebotId: "typebot-id",
      blockId: "block-id",
    });
  ...
  it("falls back to the dashboard for off-origin redirects", () => {
    const input = googleSheetsOAuthContextSchema.parse({
      redirectUrl: "https://evil.example/chatbots/typebot-id/edit",
      workspaceId: "workspace-id",
    });
    ...
    expect(parseGoogleSheetsOAuthState(state).redirectPath).toBe("/chatbots");
  });
